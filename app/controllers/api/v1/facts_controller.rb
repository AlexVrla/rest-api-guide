...
before_action :find_fact, only: [:show, :update, :destroy]
  ...
  def create
    ...
    if @fact.save
      ...
      render error: { error: 'Unable to create a fact'}, status: 400
    end
  end
  ...
  def destroy
    if @fact
      @fact.destroy
      render json: {message: 'Fact succesfully destroyed'}, status: 200
    else
      render json: {message: 'Fact not found.'}, status: 400
    end
  end
